...
def build_network(lr=None, n_stack=None, image_size=None, n_actions=None):
    ...
    with tf.name_scope('act_prob_'+flag):
        ...
        all_act = tf.contrib.layers.fully_connected(f, n_actions)
        all_act_prob = tf.nn.softmax(all_act, name='act_prob_'+flag)  # use softmax to convert to probability

    with tf.name_scope('loss_'+flag):
        # to maximize total reward (log_p * R) is to minimize -(log_p * R), and the tf only have minimize(loss)
        # this is negative log of chosen action
        neg_log_prob = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=all_act, labels=tf_actions)
        # or in this way:
