...
def build_network(lr, two_fl=True, n_features=None, n_actions=None):
    ...
    #     # (?, 4, 80, 80)
    #     e_input = tf.transpose(eval_net_input, [0, 2, 3, 1])  # (?, 80, 80, 4)
    #
    #     e_conv1 = tf.nn.relu(
    ...
    #     # (?, 4, 80, 80)
    #     t_input = tf.transpose(target_net_input, [0, 2, 3, 1])  # (?, 80, 80, 4)
    #
    #     t_conv1 = tf.nn.relu(
    ...
    with tf.variable_scope('eval_net_' + flag):
        ...
        if two_fl is True:
            ...
            q_eval_out = tf.layers.dense(e_f2, n_actions, kernel_initializer=w_initializer,
                                         bias_initializer=b_initializer, name='q_e_' + flag)
        else:
            q_eval_out = tf.layers.dense(e_f1, n_actions, kernel_initializer=w_initializer,
                                         bias_initializer=b_initializer, name='q_e_' + flag)

    with tf.variable_scope('loss_' + flag):
        loss = tf.reduce_mean(tf.squared_difference(q_target, q_eval_out, name='TD_error_' + flag))
    ...
    with tf.variable_scope('target_net_' + flag):
        ...
        if two_fl is True:
            ...
            q_target_out = tf.layers.dense(t_f2, n_actions, kernel_initializer=w_initializer,
                                           bias_initializer=b_initializer, name='t_e_' + flag)
        else:
            q_target_out = tf.layers.dense(t_f1, n_actions, kernel_initializer=w_initializer,
                                           bias_initializer=b_initializer, name='t_e_' + flag)
